...
import com.snowgears.domination.util.tabbed.item.BlankTabItem;
...
public class TableTabList extends SimpleTabList {
    ...
    public TableTabList(Player player, int columns, int minColumnWidth, int maxColumnWidth) {
        super(player, -1, minColumnWidth, maxColumnWidth);
    ...
    public boolean fill(int col1, int row1, int col2, int row2, List<TabItem> items, TableCorner startCorner, FillDirection direction) {
        ...
        //TabbedManager.log(Level.INFO, "Filling " + col1 + "," + row1 + "->" + col2 + "," + row2 + " with " + map.size() + " items");
        set(map);
